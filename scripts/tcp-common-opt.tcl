...
TCP_pair instproc init_delay { } {
    $self instvar tcps tcpr;
    global enable_delay use_deadline packet_properties_assigner_filename;

    if {$enable_delay} {
        $tcps enable-delay
        $tcpr enable-delay
        
        if {[string compare "$packet_properties_assigner_filename" "NA"] != 0} {
            $tcps delay-command set-packet-properties-assigner $packet_properties_assigner_filename
            $tcpr delay-command set-packet-properties-assigner $packet_properties_assigner_filename
        }

        # TODO: remove
        $tcps delay-command set-maximum-slack 6.4e-4
        $tcpr delay-command set-maximum-slack 6.4e-4
    }

    $tcps set use_deadline $use_deadline
    $tcpr set use_deadline $use_deadline
}

TCP_pair instproc setup {snode dnode} {
#Directly connect agents to snode, dnode.
#For faster simulation.
    global ns link_rate myAgent delay_assigner_filename packet_delay_filename enable_delay load
    $self instvar tcps tcpr;# Sender TCP,  Receiver TCP
    ...
    if {[string compare $delay_assigner_filename "NA"] != 0} {
        $tcps set-delay-assigner "$delay_assigner_filename"
        $tcpr set-delay-assigner "$delay_assigner_filename"
    }
...
TCP_pair instproc fin_notify {} {
    ...
    if { [info exists aggr_ctrl] } {
        $aggr_ctrl $fin_cbfunc $pair_id $bytes $dt $bps [expr $rttimes - $old_rttimes] [$tcps set nominal_deadline] [$tcps set early_terminated_]
    }
}
...
Agent_Aggr_pair instproc set_PUarrival_process {lambda min_bytes max_bytes rand1 rand2} {
#public
#fixed interval
#fixed flow size
  ...
  $self set rv_nbytes [new RandomVariable/Uniform]
  $rv_nbytes use-rng $rng2
  $rv_nbytes set min_ $min_bytes
  $rv_nbytes set max_ $max_bytes
}
...
Agent_Aggr_pair instproc fin_notify { pid bytes fldur bps rttimes deadline early_terminated } {
#Callback Function
#pid  : pair_id
...
#
    ...
    if { [info exists logfile] } {
        ...
        puts $logfile "$tmp_pkts $fldur $rttimes $group_id $deadline $early_terminated"
        flush $logfile
    }
