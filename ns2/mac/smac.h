...
// MAC states
enum class SMacState {
      SLEEP = 0         // radio is turned off, can't Tx or Rx
    , IDLE = 1         // radio in Rx mode, and can start Tx
//#define CHOOSE_SCHED 2  // node in boot-up phase, needs to choose a schedule
    , CR_SENSE = 2      // medium is free, do it before initiate a Tx
//#define BACKOFF 3       // medium is busy, and cannot Tx
    , WAIT_CTS = 3      // sent RTS, waiting for CTS
    , WAIT_DATA = 4     // sent CTS, waiting for DATA
    , WAIT_ACK = 5      // sent DATA, waiting for ACK
#ifdef JOURNAL_PAPER
    , TX_NEXT_FRAG = 6 // send one fragment, waiting for next from upper layer
#else
    , WAIT_NEXTFRAG = 6 // send one fragment, waiting for next from upper layer
#endif

#ifdef JOURNAL_PAPER
    , DATA_SENSE1 = 7 // received a RTS destined to another node, keep listening until confirm sender gets a CTS or starts tx data
    , DATA_SENSE2 = 8 // received a RTS destined to another node,and did not receive a RTS, keep listening until timeout or receive data
    , TX_PKT = 9 // before sending CTS/DATA/ACK, need to wait for a sifs_ time
#endif
};
