...
int SmacTimer::busy()
{
	if (status_ != TimerStatus::PENDING)
		return 0;
	else
		return 1;
}
...
void SMAC::handleGeneTimer() 
{
	...
	if (state_ == SMacState::WAIT_CTS) {  // CTS timeout
	...
	} else if (state_ == SMacState::WAIT_ACK) { // ack timeout
...
#ifdef JOURNAL_PAPER
        } else if (state_ == SMacState::DATA_SENSE1) {
                state_ = SMacState::DATA_SENSE2;
                mhGene_.resched(timeWaitCtrl_);
                                                                                                                                                            
        } else if (state_ == SMacState::DATA_SENSE2) {
                state_ = SMacState::IDLE;
...
int SMAC::checkToSend() {
	...
	if (txData_ == 1) {
		...
		if (state_ != SMacState::SLEEP && state_ != SMacState::IDLE && state_ != SMacState::WAIT_DATA )
			goto done; // cannot send if not in any of these states
    
		if (!(mhNav_.busy()) && !(mhNeighNav_.busy()) &&
		    (state_ == SMacState::SLEEP || state_ == SMacState::IDLE)) {
      
			if (state_ == SMacState::SLEEP) wakeup();
...
void SMAC::handleNeighNavTimer() {
	...
	if (state_ == SMacState::WAIT_DATA) { // data timeout
		state_ = SMacState::IDLE;
...
void SMAC::handleCounterTimer(int id) {
	...
	sched_2:
		...
		if (radioState_ == RADIO_RX)
			goto sched_3;
#ifdef JOURNAL_PAPER    
		if (id == 0 && state_ == SMacState::IDLE && searchNeighb_ ==0 && adaptiveListen_ ==0 )
#else
		if (id == 0 && state_ == SMacState::IDLE && searchNeighb_ ==0 )
#endif
			sleep();
...
void SMAC::handleCTS(Packet *p) {
	...
	if(cf->dstAddr == index_) { // for me
		if(state_ == SMacState::WAIT_CTS && cf->srcAddr == sendAddr_) {
			// cancel CTS timer
			mhGene_.cancel();

			if(sendDATA()) {
				state_ = SMacState::WAIT_ACK;
#ifndef JORNAL_PAPER
				if (!syncFlag_)
					txData_ = 0;
				else
		...
		updateNav(cf->duration);
#ifdef JOURNAL_PAPER
                if(state_ == SMacState::DATA_SENSE1 || state_ == SMacState::DATA_SENSE2) { mhGene_.cancel();}
                if(state_ == SMacState::IDLE || state_ == SMacState::CR_SENSE || state_ == SMacState::DATA_SENSE1 || state_ == SMacState::DATA_SENSE2)
                        sleep();
#else
		if(state_ == SMacState::IDLE || state_ == SMacState::CR_SENSE)
			sleep();
#endif
	}
}
...
void SMAC::handleDATA(Packet *p) {
	...
	if((u_int32_t)sh->dstAddr == MAC_BROADCAST) { // brdcast pkt
		...
		if(state_ == SMacState::WAIT_DATA && sh->srcAddr == recvAddr_) {
...
#endif
		} else if (state_ == SMacState::IDLE || state_ == SMacState::CR_SENSE ) {
		...
		Packet::free(p);
#ifdef JOURNAL_PAPER
                if (state_ == SMacState::DATA_SENSE2) { mhGene_.cancel();}
                if (state_ == SMacState::IDLE || state_ == SMacState::CR_SENSE || state_ == SMacState::DATA_SENSE2)
                        sleep();
#else
		if (state_ == SMacState::IDLE || state_ == SMacState::CR_SENSE)
			sleep();
...
void SMAC::handleACK(Packet *p) {
	...
	if (cf->dstAddr == index_) {
		if (state_ == SMacState::WAIT_ACK && cf->srcAddr == sendAddr_) {
			// cancel ACK timer
			mhGene_.cancel();
#ifdef JOURNAL_PAPER
                        numFrags_--;
...
bool SMAC::txNextFrag(void* data)
{
      // Send subsequent fragments
        ...
        if (state_ != SMacState::TX_NEXT_FRAG || data == 0) return 0;
