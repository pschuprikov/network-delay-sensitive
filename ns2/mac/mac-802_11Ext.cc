...
void Mac802_11Ext::log(char const* event, char const* additional) {
	if (MAC_DBG)
...
void ChannelStateMgr::setIFS(double tifs) {
	...
	if (ifsTimer_->status()==TimerStatus::PENDING) // ifsTimer has been scheduled
	{
		ifsTimer_->cancel();
...
void ChannelStateMgr::handlePHYBusyIndication() {
	...
	case noCSNAV:
		...
		break;
	case WIFS:
		if (ifsTimer_->status()==TimerStatus::PENDING)
			ifsTimer_->cancel();
		setChannelState(CSnoNAV);
		break;
	default:
