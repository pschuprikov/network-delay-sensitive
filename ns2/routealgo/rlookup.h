...
typedef std::pair<nodeid_t, nodeid_t> LookupPair_t; // Node/NextHop Pair
...
public :
  ...
  virtual void Populate(  // Popluate the table
                        ...
                        = 0;
  virtual void Populate(std::istream& is);           // Populate from log
  ...
  virtual void     Log(std::ostream&);              // Log to ostream
...
#ifdef USE_HASH_MAP
typedef std::hash_map<nodeid_t, nodeid_t,
                 hash<nodeid_t>, std::equal_to<nodeid_t> > RouteMap_t;
#else
typedef std::map<nodeid_t, nodeid_t,  std::equal_to<nodeid_t> > RouteMap_t;
#endif
