...
TcpAgent::TcpAgent()
	: super(PT_TCP),
	  t_seqno_(0), dupacks_(0), curseq_(0), 
      highest_ack_(0), 
      ...
      use_rtt_(0),
      frto_(0), 
      prev_highest_ack_(-10)
{
    if constexpr(std::is_base_of_v<TracedVar, TracedInt>) {
...
//    delay_bind_init_one("control_increase_");
    noFastRetrans_ = false; // TODO: check
//	delay_bind_init_one("noFastRetrans_");
    precision_reduce_ = true;
...
TcpAgent::reset()
{
	...
	recover_ = 0;
	closed_ = 0;
	last_cwnd_action_ = CongestionWindowAdjustmentReason::NONE;
...
void TcpAgent::output(int seqno, TcpXmissionReason reason)
{
	int force_set_rtx_timer = 0;
	Packet* p = allocpkt();
	hdr_tcp *tcph = hdr_tcp::access(p);
	hdr_flags* hf = hdr_flags::access(p);
	[[maybe_unused]] hdr_ip *iph = hdr_ip::access(p);
...
//			hf->cong_action() = 1;
	...
	if (!(rtx_timer_.status() == TimerStatus::PENDING) || force_set_rtx_timer)
 ...
 */
void TcpAgent::send_much(int force, TcpXmissionReason reason, int maxburst)
{
	send_idle_helper();
	int win = window();
	int npackets = 0;
	...
	if (burstsnd_timer_.status() == TimerStatus::PENDING)
		return;
...
#endif // WITH_QS
		} else if (!(delsnd_timer_.status() == TimerStatus::PENDING)) {
			/*
 ...
 */
void TcpAgent::reset_rtx_timer(bool is_mild, int backoff)
{
	if (backoff)
		rtt_backoff();
	set_rtx_timer();
...
void TcpAgent::opencwnd()
{
	...
	if (cwnd_ < ssthresh_) {
		...
		case 1:
			...
			if ((last_cwnd_action_ == CongestionWindowAdjustmentReason::NONE||
...
void
TcpAgent::slowdown(TcpSlowdownFlags how)
{
	...
	if (how & (TcpSlowdownFlags::CWND_HALF|TcpSlowdownFlags::CWND_RESTART|TcpSlowdownFlags::CWND_INIT
...
void TcpAgent::ecn(int seqno)
{
	if (seqno > recover_ ||
	    last_cwnd_action_ == CongestionWindowAdjustmentReason::TIMEOUT) {
		recover_ =  maxseq_;
		last_cwnd_action_ = CongestionWindowAdjustmentReason::ECN;
		if (cwnd_ <= 1.0) {
			if (ecn_backoff_)
				rtt_backoff();
			else {
		...
		if (ecnhat_) {
			...
			slowdown(TcpSlowdownFlags::CWND_ECNHAT|TcpSlowdownFlags::SSTHRESH_ECNHAT);
		} else {
            slowdown(TcpSlowdownFlags::CWND_HALF | TcpSlowdownFlags::SSTHRESH_HALF);
        }
		++necnresponses_ ;
    }
}
...
TcpAgent::send_one()
{
	if (t_seqno_ <= highest_ack_ + wnd_ && t_seqno_ < curseq_ &&
		...
		output(t_seqno_, TcpXmissionReason::NORMAL);
		if (QOption_)
			process_qoption_after_send () ;
		t_seqno_ ++ ;
		// send_helper(); ??
...
int TcpAgent::lossQuickStart()
{
       if (qs_window_ && tcp_qs_recovery_) {
		...
		// reset ssthresh to half of W-D/2?
                qs_window_ = 0;
                output(last_ack_ + 1, TcpXmissionReason::DUPACK);
                return 1;
       }
       return 0;
}
	...
	if (tno == TCP_TIMER_RTX) {
		...
		if (highest_ack_ == maxseq_ && !slow_start_restart_) {
			...
			if (highest_ack_ == maxseq_ && restart_bugfix_)
				...
				* down ssthresh_.
				*/
				slowdown(TcpSlowdownFlags::CWND_ONE | TcpSlowdownFlags::NO_OUTSTANDING_DATA);
			else if (highest_ack_ < recover_ &&
			  last_cwnd_action_ == CongestionWindowAdjustmentReason::ECN) {
			       /*
		...
		if (highest_ack_ == maxseq_ && restart_bugfix_) {
			...
			reset_rtx_timer(false, 1);
		}
		last_cwnd_action_ = CongestionWindowAdjustmentReason::TIMEOUT;
		send_much(0, TcpXmissionReason::TIMEOUT, maxburst_);
	}
	else {
		timeout_nonrtx(tno);
	}
	...
	if (!EnblRTTCtr_) {
		if (tcp_now - T_last >= rto) {
			// The sender has been idle.
		 	slowdown(TcpSlowdownFlags::SSTHRESH_THREE_QUATER|TcpSlowdownFlags::TCP_IDLE) ;
			for (int i = 0 ; i < (tcp_now - T_last)/rto; i ++) {
				slowdown(TcpSlowdownFlags::CWND_HALF_WITH_MIN|TcpSlowdownFlags::TCP_IDLE);
			}
			T_prev = tcp_now ;
			W_used = 0 ;
		}
		...
		if (t_seqno_ == highest_ack_+ window()) {
			...
			if (tcp_now - T_prev >= rto) {
				...
				slowdown(TcpSlowdownFlags::SSTHRESH_THREE_QUATER|TcpSlowdownFlags::TCP_IDLE);
				slowdown(TcpSlowdownFlags::CWND_HALF_WAY|TcpSlowdownFlags::TCP_IDLE);
				T_prev = tcp_now ;
				W_used = 0 ;
			}
		}
...
void TcpAgent::trace_event(char const * eventtype)
{
	if (et_ == NULL) return;
	int seqno = t_seqno_;
	char *wrk = et_->buffer();
