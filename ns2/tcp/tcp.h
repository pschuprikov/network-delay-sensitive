...
#include <delay_cc/tcp/mixins/BindCachingMixin.h>
#include "agent.h"
#include "tcp_header.h"

/* these are reasons we adjusted our congestion window */

enum class CongestionWindowAdjustmentReason {
    NONE = 0,
    DUPACK	= 1,
    TIMEOUT	= 2,	// retransmission timeout
    ECN	= 3,	    // ECN bit [src quench if supported]
    EXITED = 4,     // congestion recovery has ended
};


/* these are bits for how to change the cwnd and ssthresh values */

enum class TcpSlowdownFlags : uint32_t {
    CWND_HALF             = 0x00000002,
    CWND_HALF_WAY 	      = 0x00000100,
    CWND_RESTART          = 0x00000004,
    CWND_INIT             = 0x00000008,
    CWND_ONE              = 0x00000010,
    CWND_HALF_WITH_MIN    = 0x00000200,
    CWND_ECNHAT           = 0x00002000,
    SSTHRESH_HALF         = 0x00000001,
    SSTHRESH_THREE_QUATER = 0x00000080,
    SSTHRESH_ECNHAT       = 0x00001000,
    TCP_IDLE		      = 0x00000400,
    NO_OUTSTANDING_DATA   = 0x00000800
};

inline TcpSlowdownFlags operator|(TcpSlowdownFlags m1, TcpSlowdownFlags m2) {
    return TcpSlowdownFlags(static_cast<uint32_t>(m1) | static_cast<uint32_t>(m2));
}

inline bool operator&(TcpSlowdownFlags m1, TcpSlowdownFlags m2) {
    return bool(static_cast<uint32_t>(m1) & static_cast<uint32_t>(m2));
}
...
class TcpAgent : public BindCachingMixin<Agent> {
	friend class XcpEndsys;
    using super = BindCachingMixin;
protected:
    using TracedInt = int;
    using TracedDouble = double;
...
public:
	...
	void reset() override;
	void trace(TracedVar* v) override;
...
protected:
    ...
    virtual void output(int seqno, TcpXmissionReason reason = TcpXmissionReason::NORMAL);
	virtual void send_much(int force, TcpXmissionReason reason, int maxburst = 0);
	...
	virtual void slowdown(TcpSlowdownFlags how);
	...
	CongestionWindowAdjustmentReason last_cwnd_action_;	/* CWND_ACTION_{TIMEOUT,DUPACK,ECN} */
	...
	void reset_rtx_timer(bool is_mild, int backoff = 1);
	...
	int pipe_prev_; /* window size when timeout last occurred */
        ...
        //EventTrace *et_;
    void trace_event(char const * eventtype) override;
...
class RenoTcpAgent : public virtual TcpAgent {
 ...
 protected:
	int allow_fast_retransmit(CongestionWindowAdjustmentReason last_cwnd_action_);
...
class NewRenoTcpAgent : public virtual RenoTcpAgent {
 ...
 protected:
	...
	int allow_fast_retransmit(CongestionWindowAdjustmentReason);
...
protected:
	...
	void output(int seqno, TcpXmissionReason reason) override;
