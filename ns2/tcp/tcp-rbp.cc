...
void
RBPVegasTcpAgent::send_much(int force, TcpXmissionReason reason, int maxburst)
{
...
RBPVegasTcpAgent::paced_send_one()
{
	if (rbp_mode_ == RBP_GOING && able_to_rbp_send_one()) {
		...
		// send one packet
		output(t_seqno_++, TcpXmissionReason::RBP);
...
void
RBPRenoTcpAgent::send_much(int force, TcpXmissionReason reason, int maxburst)
{
	if (rbp_mode_ == RBP_POSSIBLE && able_to_rbp_send_one()) {
		// start paced mode
		rbp_mode_ = RBP_GOING; 
