	...
	if (tcph->seqno() > last_ack_) {
		...
		if(v_worried_>0) {
			...
			if(expired>=0) {
				...
				output(expired, TcpXmissionReason::DUPACK);
			} else
				v_worried_ = 0;
		}
...
VegasTcpAgent::timeout(int tno)
{
	if (tno == TCP_TIMER_RTX) {
		 ...
		 * phase effect. */
		send_much(1, TcpXmissionReason::TIMEOUT);
	}
}

void
VegasTcpAgent::output(int seqno, TcpXmissionReason reason)
{
	Packet* p = allocpkt();
