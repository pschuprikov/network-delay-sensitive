...
FullTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	if (delay_bind(varName, localName, "prio_scheme_", reinterpret_cast<std::underlying_type<PrioScheme>::type *>(&prio_scheme_), tracer)) return TCL_OK; // Shuang
 ...
 */
void
FullTcpAgent::sendpacket(int seqno, int ackno, int pflags, int datalen, TcpXmissionReason reason, Packet *p) {
    if (!p) p = allocpkt();
    ...
    if (enable_pias_) {
        ...
        if (datalen > 0) {
            if (deadline != 0 && use_deadline) {
                ...
                iph->prio() = calc_no_deadline_priority(seqno, curseq_);
            }
        }
    }

    inform_pacer_if_needed(iph, datalen > 0);
...
void FullTcpAgent::inform_pacer_if_needed(hdr_ip *iph, bool has_data) {
    if (!has_data) {
        return;
    }
 ...
 */
int
FullTcpAgent::foutput(int seqno, TcpXmissionReason reason)
{
...
send:
	...
	*/
	...
	auto reliable = datalen + syn + fin; // seq #'s reliably sent
	/*
		...
		if (tiflags & TH_ACK)
		{
                        ...
                        // if SYN/ACK packet and ecn_syn_wait_ == 2
			{
                                ...
                                // If SYN/ACK packet is CE-marked
				{
					...
					//newack(pkt);
					set_rtx_timer();
					sendpacket(t_seqno_, rcv_nxt_, TH_ACK|TH_ECE, 0, TcpXmissionReason::NORMAL);
					goto drop;
				}
	    		}
		}
...
if (t_rtt_) {
	...
	rtt_update(tao);
			...
			sendpacket(t_seqno_, rcv_nxt_, TH_ACK, 0, TcpXmissionReason::NORMAL);
		} else {
...
trimthenstep6:
	...
	case TCPS_SYN_RECEIVED:	/* want ACK for our SYN+ACK */
		...
		// Reset the rtx timer, send another SYN/ACK packet
			...
			cancel_rtx_timer();
			ecn_syn_next_ = 0;
			foutput(iss_, TcpXmissionReason::NORMAL);
			wnd_init_option_ = 1;
                        wnd_init_ = 1;
			goto drop;
		}
...
dropwithreset:
	...
	if (tiflags & TH_ACK) {
		sendpacket(ackno, 0, 0x0, 0, TcpXmissionReason::NORMAL);
	} else {
		int ack = tcph->seqno() + datalen;
		if (tiflags & TH_SYN)
			ack--;
		sendpacket(0, ack, TH_ACK, 0, TcpXmissionReason::NORMAL);
	}
drop:
   	Packet::free(pkt);
	return;
