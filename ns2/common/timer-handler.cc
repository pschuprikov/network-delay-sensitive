...
{
	if (status_ != TimerStatus::PENDING)   // sanity check
		abort();
	status_ = TimerStatus::HANDLING;
	expire(e);
	// if it wasn't rescheduled, it's done
	if (status_ == TimerStatus::HANDLING)
		status_ = TimerStatus::IDLE;
}
